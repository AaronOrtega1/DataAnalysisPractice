...
class DataVisualizer:
	...
	def number_of_unique(self):
		...
		for col in self.data.columns:
			print(f"{col} Column has {self.data[col].nunique()} number of unique values.")
	...
	def barplot_percentage(self, column, title="", slice=12, log_scale=False):
		...
		try:
			...
			for p in axis.patches:
				width = p.get_width()
				height = p.get_height() # the height is equal to the total of occurences
				percentage = f"{100 * height / total:.1f}%"
				x, y = p.get_xy() # gets the coordinates of the bottom left corner of the bar
				axis.annotate(f"{percentage}", (x + width/2, y + height*1.02), ha="center") # put the annotation at the center a little bit above the top of the bar.
	...
	def oldest_latest_date(self, column):
		...
		try:
			...
			print(f"The oldest date is: {self.data[column].min()}, while the latest date is: {self.data[column].max()}")
